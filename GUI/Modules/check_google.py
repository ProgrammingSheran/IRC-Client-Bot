...
class CheckGoogle(QWidget):
    ...
    def MainMessage(self, message):
        ...
        if msg.startswith("Query") or msg.startswith("query") or msg.startswith("QUERY"):
            string = msg.split(" ")
            query = string[1]
            stopper = 10
            if len(string) > 2:
                stopper = int(string[2])

            search = googlesearch.search(query, num=stopper, stop=stopper)
            self.Main_Sock.send_message("Found following websites:")
            number = 1
            for url in search:
                self.Main_Sock.send_message("%s --> %s" % (number, url))
                number += 1
